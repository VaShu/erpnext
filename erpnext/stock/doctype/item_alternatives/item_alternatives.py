...
class ItemAlternatives(Document):
	def onload(self):
...
def check_one_way(cur_alt_item, parent_item):
	"""
		...
		params:parent_item:hash
			Doctype item field
	"""
	doc = frappe.get_all('Item Alternatives', filters={'item': cur_alt_item})
	if doc:
		doc = frappe.get_doc('Item Alternatives', doc[0]['name'])
		if doc.alt_items:
...
def check_two_way(cur_alt_item, cur_item_uom, parent_item, parent_uom):
	...
	if doc:
		...
		doc = frappe.new_doc("Item Alternatives")
	...
	if not already_set:
		...
		new_doc.parenttype = 'Item Alternatives'
...
def delete_two_way(deleted_item, parent_item):
	"""
		...
		params:parent_item:hash
			Doctype item field
	"""
	parent = frappe.get_all('Item Alternatives', filters={'item': deleted_item})
	if parent:
		parent = parent[0].name
		frappe.db.sql("""delete from `tabItem Alternative`
