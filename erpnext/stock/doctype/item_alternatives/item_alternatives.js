...
frappe.ui.form.on('Item Alternatives', {
	...
	if (row.item && row.uom && row.type) {
		...
		// This is an insanity check
		 if (row.type == __('One-Way')) {
			 ...
			 frappe.call({
				 method: 'erpnext.stock.doctype.item_alternatives.item_alternatives.check_one_way',
				 freeze: true,
		 ...
		 if (row.type == __('Two-Way')) {
			...
			var d = new frappe.ui.Dialog({
			    ...
			    primary_action: function(){
					...
					frm.refresh()
					frappe.call({
						method: 'erpnext.stock.doctype.item_alternatives.item_alternatives.check_two_way',
						freeze: true,
						freeze_message: __("Checking Relation..."),
						args: {
							 cur_alt_item: row.item,
...
frappe.ui.form.on('Item Alternative', {
	before_alt_items_remove: function(frm, cdt, cdn) {
		...
		if (deleted_row.type == __('Two-Way')) {
			frappe.call({
				method: 'erpnext.stock.doctype.item_alternatives.item_alternatives.delete_two_way',
				args: {
